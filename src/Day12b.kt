  ...
  override fun run() {
    ...
    while (positions.isNotEmpty()) {
      ...
      while (neighbors.isNotEmpty()) {
        ...
        area += 1
        if (garden[i][j] == garden[i+1][j  ]) neighbors.add(Position(i+1,j  ))
        else if (garden[i][j] != garden[i  ][j-1] || garden[i  ][j-1] == garden[i+1][j-1]) perimeter+=1

        if (garden[i][j] == garden[i-1][j  ]) neighbors.add(Position(i-1,j  ))
        else if (garden[i][j] != garden[i  ][j-1] || garden[i  ][j-1] == garden[i-1][j-1]) perimeter+=1

        if (garden[i][j] == garden[i  ][j+1]) neighbors.add(Position(i  ,j+1))
        else if (garden[i][j] != garden[i-1][j  ] || garden[i-1][j  ] == garden[i-1][j+1]) perimeter+=1
